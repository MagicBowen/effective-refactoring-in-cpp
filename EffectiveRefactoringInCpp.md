...
## 总结
Martin Fowler的《重构》一书非常经典，我基本每年都会重新翻看一遍，每次都会有新的收获! 这篇文章很大一部分是关于自己学习《重构》一书的一些心得，包括对《重构》一书的一些精炼和总结，帮助大家快速上路. 当然关于重构最精华的部分还是在原书的每一处细节中. 励志学好重构的同学对于《重构》一书最好能够反复阅读实践.
这篇文档另外一部分是关于`C++`语言重构的一些工程方面的经验总结，都是一些粗浅的工具使用和工程经验介绍，希望可以降低大家使用`C++`进行重构的实践难度.

简单设计不是设计出来的！ 保持代码符合简单设计是一项日常行为，重构是达成它的唯一方式. 对重构的使用应该融入到代码开发的每时每刻，到最后不必强行区分是在开发还是重构，就像《重构》的序言中所说"变得像空气和水一样普通". 希望每个学习重构的同学都能体会到这种感觉!
